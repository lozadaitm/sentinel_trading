...
- **OP2 (Hedge Lock) = CONGELAR la pérdida.** Ningún mecanismo debe cerrar la cobertura salvo: (a) cierres parciales del Healer, o (b) cuando el saldo neto TOTAL de la cesta sea positivo. El freeze del Profit Banking ahora evalúa el **P&L TOTAL del ciclo = `cycle_realized + basket_net`** (realizado + flotante), no solo el flotante — así una amputación del Healer no puede levantar el neto y desarmar la cobertura (cascade). Ver [[healer-unwind-hedge-cascade]].
- **Healer** hace cierres parciales con el **90% del presupuesto del CICLO** (verdes generados dentro del ciclo, no cross-ciclo; ver [[healer-budget-accumulates-across-cycles]]), sin esperar a que un solo ganador cubra toda la pérdida. Selecciona el peor leg y dimensiona por **dinero real = `profit + swap`** (considera swap, no solo profit).

...
> **CORREGIDO (commit `210b099`):** antes el Healer corría transversal cada tick y amputaba a cualquier `core_count` (incluso OP1 sola), por eso las 3 OP1 rojas de este caso. Ahora `_check_healing` exige `core_count >= healer_min_core` (default 3) → solo ampu­ta en OP3/OP4+. Con OP1 sola la cubre el Hedge Lock, no el Healer. Las OP1 rojas por Healer a core 1/2 ya no deberían ocurrir.
